...
const Chart = () => {
  ...
  const formatData = (historicalData) => {
    return Object.entries(historicalData).map(([date, value]) => ({
      date,
      value: parseFloat(value["4. close"]).toFixed(2),
    }));
  };
    ...
    const updateChartData = async () => {
      try {
        ...
        if (result && result["Time Series (Daily)"]) {
          const formattedData = formatData(result["Time Series (Daily)"]);
